module Kramdown
  module Parser
    class Automation
      ...
      FENCED_CODEBLOCK_AUTOMATION_START_PATTERN = /\s*(\w*)(?:.[{](\s*[a-zA-Z0-9]+=\".+\")+[}])?/m
      ...
      FENCED_CODEBLOCK_MATCH = /^([ ]{0,3}[~`]{3,})\s*(\w*)(?:\W*[{](\s*[a-zA-Z0-9-]+=\".+\")+[}])\W*\n([\S\W\n]*)\n\1/m
      ...
      def self.parse_codeblock_automation(base, src, tree)
        ...
        if src.check(FENCED_CODEBLOCK_MATCH_PATTERN.())
          ...
          el = base.send(:new_block_el, :codeblock, src[4], nil, location: start_line_number, fenced: true)
